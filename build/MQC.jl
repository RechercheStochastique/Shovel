...
function findwire!(mqc::MQC)
    ...
    println("We have added the pseudo in connector in the wires:")
    ...
    println("There are ", length(pseudoplugout_list), " pseudoplugout in that MQC.")
    println("\nWe have added the user defined connector into the wires:")
    for wire in mqc.wire_list
        printWire(stdout, wire)
    end
    ...
    println("We have created the pseudo circuit psi with the following definition:")
    printlightQC(stdout, psi)
    i = 0
    for wire in mqc.wire_list
        i = i + 1
